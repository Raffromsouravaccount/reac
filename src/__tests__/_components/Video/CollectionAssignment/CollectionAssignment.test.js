...
import CollectionAssignment from '../../../../_components/Video/CollectionAssignment/CollectionAssignment';
import * as commonService from '../../../../_services/common.service';
import * as videoService from '../../../../_services//videoMgmt.service';
...
const setup = (props = { match: { params: { id: contentId } }, history: { goBack: () => { } } }, state = null) => {
...
describe('<CollectionAssignment />', () => {
    let wrapper;
    beforeEach(() => {
        videoService.postCollectionAssignment = jest.fn().mockReturnValue([{}])
        videoService.deleteCollectionAssignment = jest.fn().mockReturnValue({ status: 200 })
        videoService.getCollectionAssignment = jest.fn().mockReturnValue([{}])
        videoService.rearrangeCollectionAssignment = jest.fn().mockReturnValue({ status: 200 })
    ...
    it('should test getVideoCollectionAssignmentStatus', () => {
        const testComp = wrapper.instance();
        testComp.setState({ contentId: contentId, userID: '123' })
        jest.spyOn(testComp, 'getVideoCollectionAssignmentStatus');
        testComp.getVideoCollectionAssignmentStatus();
        expect(testComp.getVideoCollectionAssignmentStatus).toHaveBeenCalledTimes(1);
    })
