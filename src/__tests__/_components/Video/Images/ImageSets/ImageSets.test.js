...
import ImageSets from "../../../../../_components/Video/Images/ImageSets/ImageSets";
...
describe("render", () => {
  ...
  it("should check uploadImagefile method",() => {
    const file = [
      {
    ...
    const signedUrl = commonService.get_signed_url_and_upload_to_s3(
      file,
      imageObj
    );
    const instance = wrapper.instance();
  ...
  it("should check updateImageSet method call", (done) => {
    const imageItem = { setIndex, imageIndex, menuItem, imageData };
    const dataParams = {
      imageSetId: "53bddd6b-c980-4a52-b88e-50fe3d941605",
      images: [{
    ...
    instance.updateImageSet(dataParams, viewEditModalState, imageItem);
    expect(instance.updateImageSet).toHaveBeenCalledTimes(1);
    moxios.wait(function () {
      const request = moxios.requests.mostRecent();
      request.respondWith(response).then((res) => {
        expect(wrapper.state().updateImageStatus).toBe();
        done();
      });
    });
  });
  ...
  it("should check the getVideoDetails ",(done) => {
    const response = {
      imageSets: [
        {
          default: true,
    ...
    const instance = wrapper.instance();
    jest.spyOn(instance, "getVideoDetails");
    instance.getVideoDetails();
    expect(instance.getVideoDetails).toBeCalled();
    moxios.wait(function () {
      const request = moxios.requests.mostRecent();
      request.respondWith(response).then((res) => {
