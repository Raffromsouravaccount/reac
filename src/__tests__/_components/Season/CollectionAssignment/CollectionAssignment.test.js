...
import CollectionAssignment from '../../../../_components/Season/CreateSeason/CollectionAssignment/CollectionAssignment';
...
const setup = (props = { match: { params: { seasonId: contentId } }, history: { goBack: () => { } } }, state = null) => {
...
describe('<CollectionAssignment />', () => {
    ...
    it('should test getSeasonCollectionAssignmentStatus', () => {
        const testComp = wrapper.instance();
        testComp.setState({ contentId: contentId, userID: '123' })
        jest.spyOn(testComp, 'getSeasonCollectionAssignmentStatus');
        testComp.getSeasonCollectionAssignmentStatus();
        expect(testComp.getSeasonCollectionAssignmentStatus).toHaveBeenCalledTimes(1);
    })
