...
class RelatedContent extends Component {
  ...
  static getDerivedStateFromProps(props, state) {
    state.movieId = props.match.params.id
    return state
  }
  ...
  getMovieRelatedContentStatus = async () => {
    const { movieId, userData } = this.state;
    const response = await apiCalls(`${Config.movieAction}/${movieId}?section=relatedContent`, "GET");
  ...
  markAsDone = async (isDone) => {
    const { movieId, markDoneEnabled } = this.state;
    if (!markDoneEnabled) {
      return false;
    }
    if (isDone) {
      const requestBody = { movieId, isDone, sectionName: 'relatedContent' };
  ...
  unLockedSession = async () => {
    ...
    const requestBody = { movieId, isLocked: true, sectionName: 'relatedContent' };
  ...
  removeContentsFromDB = async (currentTab, item) => {
    ...
    const response = await delete_related_content(requestbody, this.state.movieId, this.autoSaveError)
  ...
  reArrangeContents = async (contentList, contentData, currentTab) => {
    ...
    const response = await rearrange_related_content(requestBody, this.state.movieId, this.autoSaveError)
    if (response && response.status === 200) {
      this.markAsDone(false)
    } else {
      this.setState({ [currentTab.name]: contentList })
  ...
  render() {
    const { movieId, isDone, markDoneEnabled, options, adding, isLocked, lockedBy, commanModal, showFilterDrawer } = this.state;
    const permission = permissionObj.movies.relatedContent;
    ...
    return (
      <Fragment>
        <div className="d-wrap c-n">
          <BreadCrumbs className="" links={breadCrumbs.links(urlType, action, movieId)}
