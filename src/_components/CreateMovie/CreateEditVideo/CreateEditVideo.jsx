...
class CreateEditVideo extends Component {
  ...
  async componentDidMount () {
    ...
    await this.fillMovieVideoDetails(requestParams);
  }
  ...
  importDetails = async () => {
    ...
    const copyJSON = [...JSONSchema];
      if (response) {
        ...
        const urlSave = `${Config.createEditVideo}/${contentId}`;  
        const urlProperties = `${Config.movieProperties}/${contentId}`;    
        let modifiedHlsId = [];
        ...
        let putData = {}
        putData = { movieVideo: contentData };
        await apiCalls(urlSave, "PUT", putData, null, false, false, this.props.autoSaveError);
        if(contentDuration){
          contentDuration = new Date(contentDuration * 1000).toISOString().substr(11, 8);
          let movieData = {language, movie: { duration : contentDuration}};
          await apiCalls(urlProperties, "PUT", movieData, 'null', false, false);
        }
  ...
  fillMovieVideoDetails = async (requestParams) => {
    const { contentId } = requestParams;
    const { JSONSchema } = this.state;
    const url = `${Config.createEditVideo}/${contentId}`;
    const response = await apiCalls(url, "GET", {});
    const copyJSON = [...JSONSchema];
    if (response) {
  ...
  autoSave = async (elemIndex) => {
    ...
    if (updatedElement?.touched && updatedElement.valid ) {
      ...
      if(updatedElement.name === 'dashSuffixesId' || updatedElement.name === 'hlsSuffixesId' || updatedElement.name === 'mssSuffixesId'){
        movieVideo[updatedElement?.name] = updatedElement.value.map(data => data.id);
        putData = { movieVideo: movieVideo };
      } else {
        movieVideo[updatedElement?.name] = updatedElement?.value
        putData = { movieVideo: movieVideo };
      }
      if (updatedElement?.name === "subtitleManifest") {
        movieVideo[updatedElement.name] = updatedElement.value === 'True' ? true : updatedElement.value === 'False' ? false : null;
      }
            ...
            <div className="drag-drop-wrap">
              <div className="ccm-head flex align-items-center justify-content-between">
                ...
                <div className="status-head flex align-items-center">
                  {status && <BadgeBox className="create-movie-stage" status={status} dot={true} />}
              ...
              <div className="col-12">
                <div className="row input-space-35">
                  <FormRender
                    ...
                    serverCall={true}
                    sectionMultipleBlock={<div className="col-md-6 col-lg-6"><div className="p-b-20 p-t-10"><a onClick={() => this.importDetails()}>Import Details</a></div></div>}
