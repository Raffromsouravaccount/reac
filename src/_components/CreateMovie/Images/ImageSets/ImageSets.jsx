...
class ImageSets extends Component {
  ...
  componentDidMount() {
    this.getImageSets(this.props.contentId);
  ...
  getImageSets = async (contentId) => {
    let url = `${Config.movieImageSet}/${contentId}`;
    if (this.props.page === 'collection') {
      url = `${Config.collectionImageSet}/${contentId}`;
    }
  ...
  uploadImagefile = async (file,fileObj, setIndex, imageIndex, image,set) => {
    ...
    let imageObj={
      ...
      imagetitle: imageFormatName?.length>0 ?  imageFormatName :'movie',
  ...
  deleteSet = async (set, imageSetId, imageDelete) => {
    let url = this.props.page === 'collection' ? `${Config.collectionImageSet}` : `${Config.movieImageSet}`;
    url = `${url}/${this.props.contentId}/${imageSetId}`;
    let body = {};
  ...
  navigateToEditMode = () => {
    const { page, navToEditMode, contentId } = this.props;
    const pageUrl = page === 'collection' ? 'collection' : 'movie';
    const url = `/${pageUrl}/edit/${contentId}`;
    navToEditMode(url);
  }
  ...
  activateDeactivateUser = () => {
    ...
    const {contentId}=this.props;
    let url = this.props.page === 'collection' ? `${Config.collectionImageSet}` : `${Config.movieImageSet}`;
    url = `${url}/${contentId}/${imageSetId}/${setStatus==="1"? 0:1}`;
    this.setState({
      showStatePopup: false
    }, async () => {
  ...
  render() {
    ...
    return (
      <div className="create-movie create-images-list">
                  ...
                  <div className="edit-btn">
                    <ButtonField
                     disabled={!canUpdate()}
                      ...
                      onClick={()=> canUpdate()? this.props.handleRoute(`/movie/edit/${this.props.contentId}`): ""}
                    />
                  </div>
                </div>
              }
                ...
                {imageSets.map((set, setIndex) => (
                  <div key={setIndex} className={expanded === set.imageSetId ? "ans-ques-block active" : "ans-ques-block"} >
                    ...
                    <div className="question-list flex imageset-acc">
                      <div className="question" id={setIndex} data-test="handle-accordian-button" onClick={(e) => this.handleAccordian(set.imageSetId)} >
                        <span>
        ...
        {
          viewEditModalState &&
          <ViewEditImageModal {...this.props}
            movieId={this.props.contentId}
        ...
        {   model.showModel &&
          <CommonModel className='popup-wrap status-popup' state={model.showModel}
            ...
            showDes={true} des={model.des} id="confirmationPopupClose"
