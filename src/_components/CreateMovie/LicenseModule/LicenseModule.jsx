...
import { edit_licence_service } from "../../../_services/movie.service";
...
import { getLocalData, dateDiffDayCount, checkValidationForMarkAsDoneForLicense } from "../../../_helpers/util";
...
class LicenseModule extends Component {
  ...
  handleSingleSelect = (event, id, name, value) => {
    const { formData, filters } = this.state;
    formData[name] = value && value.DisplayName ? value.DisplayName : "";
    let { } = this.state;
    this.setState({
      ...formData,
      error: false,
  ...
  createForm = (element) => {
    ...
    switch (type) {
      case "dropDown":
        return (
          <div key={name || ""} className="col-md-12 col-lg-12">
            <SelectWithSearch
              name={name}
              ...
              error={error && currentStatus == "InActive" ? true : false}
  ...
  clearFilter = () => {
    let { filters, manualJsonSchema, copyLicenseList } = this.state;
  ...
  getFilterDrawerUI = () => {
    ...
    const { filters, statusFilter, filterButton } = this.state;
    const {
      searchVal,
      platform,
      billingType,
                ...
                <div className="inner">
                  <div className="f-filter">
                    {commonmodel_section.length && (
                      <FormRender
                        data-test='input-change-common-modal'
                    ...
                    <p className="invalid-text reason-text">{reasonErrorMsg}</p>
                  </div>
                </div>
              )
          }
