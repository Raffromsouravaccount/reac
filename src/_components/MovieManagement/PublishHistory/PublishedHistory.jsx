...
class PublishedHistoryMovie extends Component {
  constructor(props) {
    super(props);
  ...
  handlePublishHistoryRoute = () => {
    ...
    if (state === "view") {
      history.push({pathname: "/movie/view/" + contentId, state:{journeyType: location?.state?.journeyType, stage: location?.state?.stage}});
    } else if (state === "quick-filing") {
      history.push("/quick/movie/edit/" + contentId);
    } else if (state === "single-landing-page") {
      history.push("/single/movie/edit/" + contentId);
    } else {
      history.push("/movie/edit/" + contentId);
    }
  };
  componentDidMount() {
  ...
  fetchContentData = async () => {
    ...
    let url = `${Config.movieControlFields}/${contentId}`;
  ...
  fetchLicenseData = async () => {
    ...
    if (isRequestIntiate || isRequestIntiate === null) {
      ...
      let url = `${Config.movie}${Config.video.videoLicenseAudit}/${contentId}?limit=${limit}&page=${page}`;
      let response = await apiCalls(url, "GET", {});
      if(response){
  ...
  render() {
    ...
    let url = "/edit";
    if (path?.includes("/movie/view")) {
      url = "/view";
    }
    if (path?.includes("/movie/quick/edit")) {
      url = "/quick/edit";
    }
    if (path?.includes("/movie/single/edit/")) {
      url = "/single/edit";
    }
            ...
            <div className="col-12">
              <div className="row con-field-wrap p-b-20 p-t-20">
                ...
                <div className="col-md-4 label-text">
                 ...
                 <div className="col-md-8 label-val">
                  {content_obj?.contentStatus ? content_obj?.contentStatus : "NA"}
                </div>
              </div>
            </div>
...
export default PublishedHistoryMovie;