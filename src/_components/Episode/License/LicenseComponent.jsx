...
class LicenseModule extends Component {
  constructor(props) {
    ...
    let { currentTabData, stage, seasonId, tvShowId ,episodeId, viewOnly } = this.props;
    this.state = {
      tvShowId: tvShowId,
      seasonId: seasonId,
      episodeId:episodeId,
  ...
  async componentDidMount() {
    const { tvShowId, seasonId,episodeId } = this.state;
  ...
  handleEditStatus = (status, licenseData, deleteLicense = false) => {
    ...
    const { formData, tvShowId } = this.state;
  ...
  deleteLicence = async () => {
    const { licence_uuid, episodeId, seasonId, tvShowId } = this.state
    const url = `${Config.episode.license}/${episodeId}/${licence_uuid}`;
  ...
  activateDeactivateLicence = async () => {
    ...
    if (currentStatus === "Active") {
      const url = `${Config.episode.license}/${tvShowId}/${seasonId}/${episodeId}/${licence_uuid}`;
  ...
  applyFilterForDrawer = (event) => {
    const movieId = this.state.tvShowId;
    const copyJSON = [...this.state.manualJsonSchema];
  ...
  render() {
    ...
    let { canUpdate } = permissionObj?.episode?.licenceModule;
    return (
              ...
              <div className="no-lice-box flex align-items-center justify-content-center">
                <div className="flex align-items-center">
                  ...
                  {expiredLicense && expiredLicense.length > 0
                    ...
                    : constantText.create_license_message_text_episode}
                </div>
              </div>
            )}
          </div>
