...
class RelatedContent extends Component {
  ...
  getRelatedContent = async () => {
    ...
    let url = `${Config.episode.relatedContent}/${this.state.episodeId}/${currentTab.name}`;
    if (currentTab.name == "rearrangeContent") {
      url = `${Config.episode.relatedRearrangeContent}/${this.state.episodeId}`;
    } else if (currentTab.name == "displayName") {
      url = `${Config.episode.relatedContent}/trailTitle/${this.state.episodeId}`;
    }
  ...
  componentDidMount = async () => {
    ...
    const res = await Promise.all([
      ...
      this.getEpisodeRelatedContentStatus(),
      this.getAllLanguage()
    ])
  ...
  getEpisodeRelatedContentStatus = async () => {
    const { episodeId, userData } = this.state;
    const response = await apiCalls(`${Config.episode.action}/${episodeId}?section=${pageSectionName}`, "GET");
  ...
  markAsDone = async (isDone) => {
    const { episodeId, markDoneEnabled } = this.state;
    if (!markDoneEnabled) {
      return false;
    }
    if (isDone) {
      const requestBody = { episodeId, isDone, sectionName: pageSectionName };
  ...
  unLockedSession = async () => {
    ...
    const requestBody = { episodeId, isLocked: true, sectionName: pageSectionName };
  ...
  addDisplayNameHandler = async (newContents) => {
    ...
    const url = `${Config.episode.relatedContent}/trailTitle`;
  ...
  updateRelatedContentRearrangeContent = async (contentList, contentData, currentTab) => {
    ...
    const url = `${Config.episode.relatedRearrangeContent}`;
  ...
  updateRelatedContents = async (newContents, contentData, currentTab) => {
    ...
    const url = `${Config.episode.relatedContent}`;
  ...
  removeContentsFromDB = async (currentTab, item) => {
    ...
    const url = `${Config.episode.relatedContent}/${this.state.episodeId}`;
  ...
  reArrangeContents = async (contentList, contentData, currentTab) => {
    ...
    const url = `${Config.episode.relatedContent}/rearrange/${this.state.episodeId}`;
  ...
  render() {
    const { episodeId, isDone, markDoneEnabled, options, adding, isLocked, lockedBy, commanModal, showFilterDrawer } = this.state;
    const permission = permissionObj.episode.relatedContent;
    // validate episodeId
    if (!episodeId) {
      return null;
    }
    ...
    return (
      <Fragment>
        <div className="d-wrap c-n">
          <BreadCrumbs className="" links={breadCrumbs.links(tvShowUrl, seasonUrl, episodeUrl, match.params.id, match.params.seasonId, episodeId)}
