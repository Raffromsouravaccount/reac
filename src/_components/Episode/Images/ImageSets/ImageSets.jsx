...
class ImageSets extends Component {
  constructor(props) {
    super(props);
    let { jsonData, stage, externalId } = props;
    this.state = {
  ...
  componentDidMount() {
    this.getImageSets(this.props.episodeId);
  }

  static getDerivedStateFromProps = (props, state) => {
    if (props.externalId != state.externalId) { state.externalId = props.externalId; }
    if (props.stage != state.status) { state.status = props.stage; }
    return state
  }

  getImageSets = async (episodeId) => {
    let url = `${Config.episode.imageSet}/${episodeId}`;
  ...
  uploadImagefile = async (file,fileObj, setIndex, imageIndex, image,set) => {
    ...
    let imageObj={
      ...
      imagetitle:imageFormatName?.length>0 ?  imageFormatName :'episode',
  ...
  updateImageSet = async (dataParams) => {
    ...
    let url =`${Config.episode.imageSet}`;
    url = `${url}/${episodeId}`;
  ...
  activateDeactivateUser = () => {
    ...
    const {episodeId}=this.props;
    let url = `${Config.episode.imageSet}`;
    url = `${url}/${episodeId}/${imageSetId}/${setStatus==="1"? 0:1}`;
    this.setState({
      showStatePopup: false
    }, async () => {
  ...
  render() {
    const {
      ...
      viewEditModalState, viewEditModalMode, imageItem, model, menuItems, showLockPopup, status, updatedImageItem ,externalId,showStatePopup,setStatus} = this.state;
    ...
    let { canUpdate }= permissionObj?.episode?.imagesModule;

    return (
      <div className="create-movie create-images-list">
        <div className="whitebox">
          <Locked
            data-test="toggleLock" id="toggleLock"
            lock={isLocked}
            lockedBy={lockedBy}
              ...
              {isViewMode &&
                <div className="status-head flex align-items-center">
                  ...
                  <div className="edit-btn">
                    <ButtonField
                     disabled={!canUpdate()}
                      buttonText={constantText.tv_show_episode_text.editEpisode}
                      className="zee-btn-field zee-full MuiButton-containedPrimary"
                      onClick={()=> canUpdate()? this.props.handleRoute(`${Config.tvShow.edit}/${this.props.tvShowId}/season/${this.props.seasonId}/episode/${this.props.episodeId}`): ""}
                    />
                  </div>
                </div>
              }
            ...
            <div className="movieForm-tab">
              {!isViewMode  &&
                <div className="col-12 flex justify-content-between align-items-center">
                  ...
                  <button className="upload-btn" disabled={isLocked} onClick={() => !isLocked ? navToImageSet(1) : ''}>
        ...
        <CommonModel className='popup-wrap status-popup' state={showLockPopup} id="common-model"
