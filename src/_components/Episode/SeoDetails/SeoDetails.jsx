...
class SeoDetails extends Component {
  constructor(props) {
    super(props);
    let { state, journeyType, jsonData } = props;
    let JSONSchema= (journeyType == "2" )? jsonData: (journeyType == "3") ? jsonData :jsonData.SeoJson
    this.state = {
      JSONSchema: DEFAULT_JSON(JSONSchema?.map(data=> ({...data}))),        
         JSONSchemaGlobal:
        DEFAULT_JSON(
          state? jsonData: jsonData?.globalProperties
        ) || [],
      tvShowId: null,
      allGlobalData: [],
      updateId: [],
  ...
  async componentDidMount() {
    let { tvShowId, seasonId, episodeId } = this.props;
    this.setState({ tvShowId: tvShowId, seasonId : seasonId, episodeId: episodeId || null }, this.fillSeoDetails);
    let shallowArr = []
  ...
  generateUrlStructure = async (response) => {
    ...
    if (response) {
      let episodeName = response?.title?.value?.toLowerCase().split(" ").join("-");
          episodeName =   episodeName.replace(constantText.seoSefUrlRegex, '');
      let externalId = response?.externalId?.value;
      const autoGeneratedUrl = `${Config.episodeSefUrl}/${episodeName}/${externalId}`;
  ...
  fillSeoDetails = async () => {
    const { tvShowId, JSONSchema, seasonId, episodeId } = this.state;
    const url = `${Config.episode.seo}/${tvShowId}/${seasonId}/${episodeId}`;
  ...
  handleSave = async (rootIndex, index, sectionName) => {
    ...
    let { errorText } = field;
    if (!errorText) {
      const response = await apiCalls(`${Config.episode?.seo}/${episodeId}`, "PATCH", updatedData, null, false, false,
  ...
  render() {
    ...
    let { canUpdate } = permissionObj?.episode?.seoModule;
