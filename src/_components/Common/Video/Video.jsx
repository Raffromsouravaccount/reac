...
const video = ({ classes = 'relatedcontent-block', data, showCheckbox, checkBoxHandler, showDelete, showButton, buttonText, deleteHandler, draggable, buttonHandler, isLocked, isViewMode = false,listView=true }) => {
 const stage = data?.contentState_populated?.title;
  const stageColour = getStageColour(stage);
  processLicenceCountries(data);
  const videoLicensesCountries = (data?.countries && typeof data?.countries == 'string') ? data?.countries?.split(",") : []
  const [licensesCountriesDialog, setCommonModel] = useState(false);
  const viewVideoHandler = () => {
    const route = `/video/view/${data.videoId}`;
  ...
  return (
    ...
    { listView &&  <div className="related-m-tag">{constantText?.journeyType[data?.journeyType]}</div>}
     { listView && <LicenseBadge data={data} handleRouteExpiredLink={viewVideoHandler} />}
      <div className="left-wrap flex">
      {showCheckbox ?
        <div className="l-chkbox">
          <Checkbox
            className={`zee-checkbox-field auto-checkBox-${data?.videoId}`}
      ...
      <div className="right-block">
        <div className="flex">
          ...
          {showDelete && !isViewMode ?
            <span id="deleteHandlerBtn" className={`remove ${isLocked ? "disabled" : ""} auto-delete-${data?.videoId}`} onClick={(event) => deleteHandler(event, data)}>
              <Delete />
            </span>
            : null}
          {showButton && !isLocked ?
            <div className="user-head">
              <div className="s-form flex justify-content-between col-md-12">
                <div id="buttonHanlder"
                  className={`btn-create-user ${data?.isChecked || isLocked ? "disabled" : ""} `}
                  onClick={() => { !data?.isChecked ? buttonHandler(data) : null }}>
                  {buttonText}
                </div>
              </div>
            </div>
    ...
    {licensesCountriesDialog &&
      <CommonModel
        ...
        showIcon={false}
        showDes={true}
        des={videoLicensesCountries.slice(2, videoLicensesCountries.length).join(", ")}
        showBtn1={false}
        showBtn2={false}
