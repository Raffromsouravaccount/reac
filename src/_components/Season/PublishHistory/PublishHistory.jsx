...
class PublishedHistorySeason extends Component {
  constructor(props) {
    super(props);
  ...
  fetchContentData = async () => {
    ...
    let url = `${Config.seasonControlFields}/${seasonId}?tvShowId=${contentId}`;
  ...
  fetchLicenseData = async () => {
    ...
    if (isRequestIntiate || isRequestIntiate === null) {
      ...
      let url = `${Config.seasonConst}${Config.video.videoLicenseAudit}/${seasonId}?limit=${limit}&page=${page}`;
      let response = await apiCalls(url, "GET", {});
      if(response){
    ...
    return (
      <Fragment>
        <div className="d-wrap c-n">
          <BreadcrumbsComp className="" links={breadCrumbs.links(tvShowUrl, seasonUrl,  contentId,match?.params?.seasonId,)}
...
export default PublishedHistorySeason;
