...
// actions
import { tvSeasonActions } from "../../../../_actions/tvSeason.action.js";
...
class CreateLicense extends Component {
  ...
  InputChanger = (event, elemIndex) => {
    let { state,journeyType } = this.props;
    const { JSON, item } = this.props || {};
    const copyJSON = [...this.state.manualJsonSchema];
  ...
  editLicence = async () => {
    const { formData, contentId, seasonId } = this.state;
    const copyJSON = [...this.state.manualJsonSchema];
    ...
    if (formValidity) {
      ...
      var response = await tvSeasonService.edit_licence_service(
        licenseId,
        contentId,
        seasonId,
        jsonObj
  ...
  submitLicence = async () => {
    ...
    if (formValidity) {
      ...
      const { seasonId, contentId } = this.state;
      const response = await tvSeasonService.create_licence_service(
        validateArr,
        contentId,
        seasonId
      );
                                ...
                                <div className="lice-box flex justify-content-between m-b-30">
                                  ...
                                  <div className="right-area">
                                    <div className="edit-btn-row flex align-items-center justify-content-end">
                                      <div className="right-area">
                                        <div className="edit-btn-row flex align-items-center justify-content-end">
                                          <div className={`default-edit-btn auto-default-edit-btn-${index}`}>
                                            <Edit
                                              onClick={() =>
                                          ...
                                          <div className={`default-delete-btn auto-default-delete-btn-${index}`}>
                                            <Delete
                                              onClick={() =>
...
const actionCreators = {
  getTemplates: tvSeasonActions.fetch_template_action,
  saveData: tvSeasonActions.create_licence_action,
  getLicenseData: tvSeasonActions.list_license_action,
  editLicence: tvSeasonActions.edit_licence_action,
};
