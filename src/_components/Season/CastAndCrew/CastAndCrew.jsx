...
class CastAndCrew extends Component {
  ...
  componentDidMount = () => {
    let { tvShowId, seasonId, viewOnly, stage } = this.props;
    if (tvShowId && seasonId) {
      this.setState(prevState => ({ viewOnly: !!viewOnly, status: stage }), async () => {
        let response = await apiCalls(`${Config.season?.castAndCrew}/${tvShowId}/${seasonId}`, "GET", {}, this.props.match.url);
  ...
  handleSave = async (rootIndex, index, sectionName) => {
    ...
    let { errorText } = field;
    if (!errorText) {
      const response = await apiCalls(`${Config.season?.castAndCrew}/${seasonId}`, "PATCH", updatedData, null, false, false,
  ...
  render() {
    const { tabData, selectedTab, actors, others, globalProperties, canMarkAsDone, showLockedPopup, viewOnly} = this.state;
                    ...
                    <div className="global-row" key={globalIndex}>
                      <div className="add-plush-row top-title">
                        ...
                        <div className="add-another-f-btn create-btn">
                          <div className={`${globalIndex > 0 ? 'remove-btn' : ''} add-btn-field flex align-items-center`}
                            onClick={() => (!isLocked && this.addRemoveMultipleFields("globalProperties", globalIndex))}>
                            {!viewOnly && <span className="plush-icon-btn"></span>}
                          </div>
                        </div>
                      </div>
                      <div className="row">
                        <ViewSeason allData={globalGroupFields}
                          ...
                          updateData={index => this.handleSave(globalIndex, index, 'globalProperties')}
                          callBack={index => this.handleEditable(globalIndex, index, "globalProperties")}
                          isDisable={!!isLocked} viewOnly={viewOnly} />
                      </div>
                    </div>
                  ))}
