...
class CollectionAssignment extends Component {
  ...
  static getDerivedStateFromProps(props, state) {
    state.contentId = props.match.params.seasonId
    return state
  }

  getAssignedCollections = async () => {
    if (!(this.state.contentId)) { return; }
    const url = `${Config.season.collectionAssignment}/${this.state.contentId}`;
  ...
  componentDidMount = async () => {
    const res = await Promise.all([
      ...
      this.getSeasonCollectionAssignmentStatus()
    ])
  ...
  getSeasonCollectionAssignmentStatus = async () => {
    const { contentId, userID } = this.state;
    const response = await apiCalls(`${Config.season.action}/${contentId}?section=${pageSectionName}`, "GET");
  ...
  markAsDone = async (isDone) => {
    ...
    if (isDone) {
      const requestBody = { seasonId: contentId, isDone, sectionName: pageSectionName };
  ...
  unLockedSession = async () => {
    ...
    const requestBody = { seasonId: contentId, isLocked: true, sectionName: pageSectionName };
  ...
  removeCollectionsFromDB = async (currentTab, item) => {
    let response = false;
    const url = `${Config.season.collectionAssignment}/${this.state.contentId}?collectionId=${item[currentTab.contentIdKey]}`;
  ...
  render() {
    ...
    const permission = permissionObj.season.collectionAssignment;
    ...
    return (
      <Fragment>
        <div className="d-wrap c-n">
          <BreadCrumbs className="" links={breadCrumbs.links(tvShowUrl, seasonUrl, match.params.id, contentId)}
