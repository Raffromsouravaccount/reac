...
import TranslationAddEdit from "./TranslationAddEdit";
import TranslationView from "../Layout/TranslationView";

import MovieContentPropertiesJson from "../Schema/Movie/ContentProperties.json";
import MovieCastNCrewJson from "../Schema/Movie/CastNCrew.json";
...
class TranslationMovie extends Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
      ...
      sections: constantText.translations.movie_sections,
      selectedSecTab: 0,
      fields: JSON.parse(JSON.stringify(MovieContentPropertiesJson)),
   ...
   * @description for default language data to be set in defaultData and for selected language data to be set in selectedLangData
   * @param {*} isDefaultData for defalult selected language (currently using English)
   */
  fetchContentData = async (isDefaultData = false) => {
    const { params, sections, selectedSecTab, language, fields, masterData } = this.state;

    const response = await translationService.get_movie_details(
      params?.movieId,
      isDefaultData ? String(Config.defaultLanguageCode) : String(language?.code),
      sections[selectedSecTab].url
    );
    ...
    if (response && response.data) {
       ...
       */
      section.url == "properties" ? fields = JSON.parse(JSON.stringify(MovieContentPropertiesJson)) : "";
      section.url == "castAndCrew" ? fields = JSON.parse(JSON.stringify(MovieCastNCrewJson)) : "";
  ...
  getMasterDataForSelect = (url) => {
    translationService.getMasterData(url).then(response => {
      if (response) {
        ...
        let { masterDataa } = this.state;
        masterDataa[url] = response;
        this.setState({ masterDataa });
      }
    })
  ...
  getMasterData = (path) => {
    if (this.state.masterDataa[path]) {
      return this.state.masterDataa[path];
    }
  ...
  getPropertiesData = async () => {
    ...
    const response = await translationService.get_movie_details(
      params.movieId, String(language?.code), 'properties'
    );
    return response;
  }
  ...
  lockUnlockTranslation = async () => {
    ...
    this.setState({
      ...
      this.lockSections(params?.movieId);
    });
  }

  lockSections = async (movieId) => {
  ...
  autoSave = async (isDoneMark) => {
    ...
    let url = "",
      ...
      paramsData = {};
      paramsData = data;
      url = `${Config.movieTranslation}/${params.movieId}?langCode=${String(language?.code)}`;
  ...
  getMovieStatus= async()=> {
    ...
    const response= await apiCalls(`${Config.movieAction}/${params?.movieId}`, "GET", null, null, false);
    if(response && response.length> 0) {
      ...
      response?.map(data=> {
        ...
        if(sName === sectionName) {
          ...
          markDone = isDone ? 3 : this.validateMarkAsDoneMovieAction();
        }
      });
...
export default TranslationMovie;
