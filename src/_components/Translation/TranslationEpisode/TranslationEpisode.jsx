...
import EpisodeContentPropertiesJson from "../Schema/Episode/ContentProperties.json";
import EpisodeGlobalContentPropertiesJson from "../Schema/Episode/GlobalContentProperties.json";
import EpisodeCastNCrewJson from "../Schema/Episode/CastNCrew.json";
...
class TranslationEpisode extends Component {
  constructor(props) {
    super(props);
    this.state = {
      ...
      sections: constantText.translations.episode_sections,
      globalSections: constantText.translations.episode_global_sections,
      selectedSecTab: 0,
      selectedGlobalSecTab: 0,
      fields: JSON.parse(JSON.stringify(EpisodeContentPropertiesJson)),
  ...
  fetchContentData = async (isDefaultData = false) => {
    ...
    if(section.url == "properties" && globalSection.url == "globalFields") {
      response = await translationService.getShowDetails('episode', params?.episodeId, lng, globalSection.url);
    } else {
      response = await translationService.getShowDetails('episode', params?.episodeId, lng, sections[selectedSecTab].url);
    }
    if (response && response.data) {
      isDefaultData ? 
        this.setState({ 
          defaultData: section.url == "properties" ? response.data[0]
       ...
       */
      section.url == "properties" ? fields = JSON.parse(JSON.stringify(EpisodeContentPropertiesJson)) : "";
      section.url == "castAndCrew" ? fields = JSON.parse(JSON.stringify(EpisodeCastNCrewJson)) : "";
      ...
      if(section.url == "properties" && globalSection.url == "globalFields") {
        fields = JSON.parse(JSON.stringify(EpisodeGlobalContentPropertiesJson));
  ...
  getPropertiesData = async () => {
    ...
    const response = await translationService.getShowDetails(
      'episode', params.episodeId, String(language?.code), 'properties'
    );
    return response;
  }
  ...
  lockUnlockTranslation = async () => {
    ...
    this.setState({
      ...
      this.lockSections(params?.episodeId);
    });
  }

  lockSections = async (episodeId) => {
  ...
  autoSave = async (isDoneMark, ...saveParams) => {
    ...
    paramsData = data;
    url = `${Config.episode[initialUrl]}/${params.episodeId}?langCode=${String(language?.code)}`;
    this.updation = true;
  ...
  getEpisodeStatus= async()=> {
    ...
    const response= await apiCalls(`${Config.episode?.action}/${params?.episodeId}`, "GET", null, null, false);
  ...
  */
  ...
  commonTabHandler = (selectedTab, tabIndex) => {
    ...
    let fields = {};
    section.url == "properties" && globalSection.url == "titleSummary" ? fields = JSON.parse(JSON.stringify(EpisodeContentPropertiesJson)) : "";
    section.url == "castAndCrew" ? fields = JSON.parse(JSON.stringify(EpisodeCastNCrewJson)) : "";
    section.url == "properties" && globalSection.url == "globalFields" ? fields = JSON.parse(JSON.stringify(EpisodeGlobalContentPropertiesJson)) : "";
    this.setState({ 
          ...
          clicked={this.toggleModel}>
            ...
            <div className="lang-f-wrap">
              {Object.keys(fields)?.map((key, levelIndex) => (
                <Fragment key={key}>
                  {(key === "actors" || key === "others" || key === 'global') && this.renderGlobalTextField(key)}
...
export default TranslationEpisode;
