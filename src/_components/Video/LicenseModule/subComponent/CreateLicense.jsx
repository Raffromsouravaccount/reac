...
import  {create_licence_service,edit_licence_service} from "../../../../_services/videoMgmt.service";
...
// for testing 
import LicenseJSON from "../../Schema/license/LicenseModule.json";
...
class ManualLicense extends Component {
  ...
  componentDidMount() {
    const {jsonData ,state,editTab}= this.props;
  ...
  fillEditDetailsTemplate = async (response, index,setName) => {
    const { contentId } = this.state;
    if (response) {
  ...
  selectGroup = (event, group) => {
    ...
    const { formValidity } = formValidityCheck(copyLicense);
    formValidButton = formValidity && formValidButton;
    let fromDate = new Date(copyLicense[0]?.value)?.getTime();
    let toDate = new Date(copyLicense[1]?.value)?.getTime();
    const { dateValidation } = checkDateValidation(fromDate, toDate);
    if (dateValidation === true) {
      formValidButton = false;
    }
  ...
  handleTemplate = (event, id, name, value) => {
    ...
    const { templateList ,licenseCount,setName,formData} = this.state;
    ...
    if (data) {
      for (let itemIndex = 0; itemIndex < data.length; itemIndex++) {
        for (
          ...
          let setNameCreate ="Set Name -".concat((setName)+(templateIndex+1))
  ...
  editLicence = async () => {
    ...
    const showId = this.state.contentId;
    const copyJSON = [...this.state.manualJsonSchema];
  ...
  editLicenceTemplate = () => {
    const { template_index, template_copyData, template,template_edit_section } = this.state;
    if(template_edit_section){
      const copyJSON = [...this.state?.template_edit_section];
      const { formValidity, validatedForm } = isValidatedForm(copyJSON);
      this.setState({
      ...
      if (formValidity) {
        ...
        template_copyData[template_index].setName = validateObj.setName;
          ...
          <div className="Rectangle-13">
            ...
            state === "quick-filing" ? (
              ...
              <div className="lice-radio-tab col-12">
                <RadioButton
                  className="zee-radio-field auto-radioManual"
                  name="manual"
                  areaLabel="manual"
                  value={licenceStatus}
                    ...
                    <div className="lice-template">
                      ...
                      {templateListData
                        ? templateListData.map((template_item, index) => (
                            <Fragment key={index}>
                              <div className="row input-space-35">
                              {console.log(template_item , "licenseDate")}
                              ...
                              <div className="lice-list">
                                <div className="lice-box flex justify-content-between m-b-30">
                                  <div className="lice-title-head">{template_item?(template_item.setName):null}</div>
                                  ...
                                  <div className="right-area">
                                    <div className="edit-btn-row flex align-items-center justify-content-end">
                                      <div className="right-area">
                                        <div className="edit-btn-row flex align-items-center justify-content-end">
                                          <div className="default-edit-btn auto-default-edit-btn">
                                            <Edit
                                              onClick={() =>
                                          ...
                                          <div className="default-delete-btn auto-default-delete-btn">
                                            <Delete
                                              onClick={() =>
...
const mapStateToProps = (state) => {
  ...
  let licenseList = state.videoMgmt_reducer.licenseRecords;
  let templateList = state.videoMgmt_reducer.templateRecords;
  return {
    masterRecords,
    licenseList,
    templateList,
...
const actionCreators = {
  getTemplates: videoMgmtActions.fetch_template_action,
  saveData: videoMgmtActions.create_licence_action,
  getLicenseData: videoMgmtActions.list_license_action,
  editLicence: videoMgmtActions.edit_licence_action,
};
