...
class ViewContentProperties extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      stage: { title: null },
      controlFieldsData: {},
      headerTabs: JSON.parse(JSON.stringify(constantText.video_content_properties_header_arr)),
      title_summary,
      specialCategory,
      specialCategoryArr,
      global,
  ...
  componentDidMount() {
    let { videoId, stage } = this.props;
    this.setState({ videoId, stage }, () => {
      if (videoId) {
  ...
  fetchContentData = async () => {
    ...
    if (response) {
      ...
      this.setState(
        ...
        () => {
          this.populateProperties(response)
  ...
  handleTab = (event, selectedTab) => {
    const headerTabs = [...this.state.headerTabs]
    if (headerTabs[selectedTab]?.label === "Control Fields" && !headerTabs[selectedTab].fetched) {
      this.getControlFieldData();
      headerTabs[selectedTab].fetched = true;
    }
    this.setState({ selectedTab, headerTabs });
  }
  ...
  render() {
    ...
    let { handleRoute } = this.props;
    let { canUpdate } = permissionObj?.videos?.contentPropertiesModule;
    return (
            ...
            <div className="status-head flex align-items-center">
              ...
              <div className="edit-btn">
                <ButtonField
                  ...
                  onClick={() => canUpdate() ? handleRoute(`/video/edit/${videoId}`) : ""}
                />
              </div>
            </div>
          </div>
