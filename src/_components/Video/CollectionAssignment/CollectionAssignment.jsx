...
import { postCollectionAssignment, deleteCollectionAssignment, getCollectionAssignment, rearrangeCollectionAssignment } from '../../../_services/videoMgmt.service';

import './CollectionAssignment.css';
...
class CollectionAssignment extends Component {
  ...
  getAssignedCollections = async () => {
    ...
    return await getCollectionAssignment({ contentId: this.state.contentId });
  }

  componentDidMount = async () => {
    const res = await Promise.all([
      this.getAssignedCollections(),
      this.getVideoCollectionAssignmentStatus()
    ])
  ...
  getVideoCollectionAssignmentStatus = async () => {
    const { contentId, userID } = this.state;
    const response = await apiCalls(`${Config.video.action}/${contentId}?section=${pageSectionName}`, "GET");
  ...
  markAsDone = async (isDone) => {
    ...
    if (isDone) {
      const requestBody = { videoId: contentId, isDone, sectionName: pageSectionName };
  ...
  unLockedSession = async () => {
    ...
    const requestBody = { videoId: contentId, isLocked: true, sectionName: pageSectionName };
    const response = await Promise.all([
      this.markAsDoneNLockedAction(requestBody),
      this.getAssignedCollections()
    ]);
    response && response[0] ? this.setState(prevState => ({ isLocked: false })) : "";
    response && response[1] && response[1].length ? this.populateDataToState(response[1]) : "";
  }
  ...
  addContentHandler = (newContents) => {
    ...
    const contentData = newContents.map(item => item = item[currentTab.contentIdKey]);
  ...
  updateCollections = async (newContents, contentData, currentTab) => {
    ...
    response = await postCollectionAssignment(this.state.contentId, requestBody, this.autoSaveError);
    if (response) {
  ...
  removeCollectionsFromDB = async (currentTab, item) => {
    const response = await deleteCollectionAssignment(item[currentTab.contentIdKey], this.state.contentId, this.autoSaveError)
  ...
  reArrangeHandler = async (event) => {
    ...
    const contentData = contentList.map(item => item = item[currentTab.contentIdKey]);
  ...
  reArrangeContents = async (contentList, contentData, currentTab) => {
    ...
    const response = await rearrangeCollectionAssignment(requestBody, this.state.videoId, this.autoSaveError)
    if (response && response.status === 200) {
      this.markAsDone(false)
    } else {
      this.setState({ [currentTab.name]: contentList })
  ...
  render() {
    ...
    const permission = permissionObj.videos.collectionAssignment;
    ...
    return (
      <Fragment>
        <div className="d-wrap c-n">
          <BreadCrumbs className="" links={breadCrumbs.links(urlType, contentId)}
