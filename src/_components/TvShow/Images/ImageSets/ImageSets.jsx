...
class ImageSets extends Component {
  ...
  getImageSets = async (tvShowId) => {
    let url = `${Config.tvShowImageSet}/${tvShowId}`;
  ...
  uploadImagefile = async (file,fileObj, setIndex, imageIndex, image,set) => {
    ...
    let imageObj={
      ...
      imagetitle:imageFormatName?.length>0 ?  imageFormatName :'tvshow',
  ...
  deleteSet = async (set, imageSetId, imageDelete) => {
    ...
    let url = `${Config.tvShowImageSet}/${this.props.tvShowId}/${imageSetId}`;
    let body = {};
  ...
  getImageListingView = (imageIndex, isLocked, setIndex, menuItems, image, pageState, isViewMode, set) => {
    ...
    const isQuickAndSingleLandingPage = !!(pageState === 'quick-filing');
  ...
  navigateToEditMode = () => {
    ...
    const url = `/${pageUrl}/edit/${tvShowId}`;
    navToEditMode(url);
  }
  ...
  activateDeactivateUser = () => {
    ...
    const {tvShowId}=this.props;
    let url = `${Config.tvShowImageSet}`;
    url = `${url}/${tvShowId}/${imageSetId}/${setStatus==="1"? 0:1}`;
    this.setState({
      showStatePopup: false
    }, async () => {
  ...
  render() {
    ...
    let { canUpdate }= permissionObj?.tvShows?.imagesModule;
    return (
                  ...
                  <div className="edit-btn">
                    <ButtonField
                     disabled={!canUpdate()}
                      ...
                      onClick={()=> canUpdate()? this.props.handleRoute(`/tvshow/edit/${this.props.tvShowId}`): ""}
                    />
                  </div>
                </div>
              }
