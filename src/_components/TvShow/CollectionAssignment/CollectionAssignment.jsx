...
class CollectionAssignment extends Component {
  ...
  getAssignedCollections = async () => {
    ...
    const url = `${Config.tvShow.collectionAssignment}/${this.state.contentId}`;
  ...
  componentDidMount = async () => {
    const res = await Promise.all([
      ...
      this.getTVShowCollectionAssignmentStatus()
    ])
  ...
  getTVShowCollectionAssignmentStatus = async () => {
    const { contentId, userID } = this.state;
    const response = await apiCalls(`${Config.tvShow.action}/${contentId}?section=${pageSectionName}`, "GET");
  ...
  markAsDone = async (isDone) => {
    ...
    if (isDone) {
      const requestBody = { tvShowId: contentId, isDone, sectionName: pageSectionName };
  ...
  unLockedSession = async () => {
    ...
    const requestBody = { tvShowId: contentId, isLocked: true, sectionName: pageSectionName };
  ...
  removeCollectionsFromDB = async (currentTab, item) => {
    let response = false;
    const url = `${Config.tvShow.collectionAssignment}/${this.state.contentId}?collectionId=${item[currentTab.contentIdKey]}`;
  ...
  render() {
    ...
    const permission = permissionObj.tvShows.collectionAssignment;
    ...
    return (
      <Fragment>
        <div className="d-wrap c-n">
          <BreadCrumbs className="" links={breadCrumbs.links(tvShowUrl, contentId)}
