...
//css
import "./PublishedHistoryTv.css";


class PublishedHistoryTvShow extends Component {
  constructor(props) {
    super(props);
  ...
  fetchContentData = async () => {
    ...
    let url = `${Config.tvShowControlFields}/${contentId}`;
  ...
  fetchLicenseData = async () => {
    ...
    if (isRequestIntiate || isRequestIntiate === null) {
      ...
      let url = `${Config.tvShowsList}${Config.video.videoLicenseAudit}/${contentId}?limit=${limit}&page=${page}`;
      let response = await apiCalls(url, "GET", {});
      if(response){
...
export default PublishedHistoryTvShow;
