...
class ManualLicense extends Component {
  ...
  handleValidateError = (templateListData) => {
    let { userDetails } = this.state;
    const dataCountry = Object.values(templateListData?.country ? templateListData?.country : []);
    var validate_Array = dataCountry.filter(function (item, index) {
  ...
  editLicence = async () => {
    ...
    if (formValidity) {
      ...
      var response = await tvShowsService.edit_licence_service(
        licenseId,
        tvShowId,
        jsonObj
      );
  ...
  submitLicence = async () => {
    ...
    if (formValidity) {
      ...
      const tvShowId = this.state.contentId;
      const response = await tvShowsService.create_licence_service(
        validateArr,
        tvShowId
      );
      if (response) {
      ...
      <div className="whitebox">
        <div className="col-12">
          <div className="Rectangle-13">
            ...
            {editTab === 1 ||
            state === "single-landing" ||
            state === "quick-filing" ? (
              <Fragment></Fragment>
            ) : (
              <div className="lice-radio-tab col-12">
                <RadioButton
                  data-test="handle-licence-change"
            ...
            {templateEdit === 1 ? (
              <form>
                ...
                <div className="col-md-6 col-lg-6 form-save-btn p-b-20">
                  <ButtonField
                    data-test="edit-license-template"
                  ...
                  <form>
                    ...
                    {templateListData && templateListData.length ? (
                      <div className="row input-space-35">
                        <div className="col-md-6 col-lg-6 form-save-btn p-b-20">
                          <ButtonField
                            data-test="submit-template-license"
                            className="zee-btn-field zee-full"
                            variant="contained"
                            color="primary"
                            buttonText={"SAVE"}
...
const actionCreators = {
  getTemplates: tvShowsActions.fetch_template_action,
  saveData: tvShowsActions.create_licence_action,
  getLicenseData: tvShowsActions.list_license_action,
  editLicence: tvShowsActions.edit_licence_action,
};
