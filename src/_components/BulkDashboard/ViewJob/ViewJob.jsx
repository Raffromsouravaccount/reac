...
class ViewJob extends Component {
  ...
  fetchJobList = async () => {
    ...
    const response = await apiCalls(
      `${Config.bulkPublishStatus}/${viewId}`,
      "GET",
      {},
      match?.url,
      false
    ...
    if (response && response?.details?.length) {
      ...
      const data = this.formatListData(response?.details);
      this.setState(
        {
          listLoading: false,
          allRows: data,
  ...
  filterData = () => {
    ...
    const filteredData = allRows.filter((item) => {
      let searchItem = `${item?.externalId} ${item?.title}`;
      return searchItem
        .toLowerCase()
      ...
      <div className="d-wrap c-n">
        ...
        <div className="user-head profile-head flex align-items-center justify-content-between">
          <div className="back-user-btn">
            <span
              onClick={() =>
                this.handleRoute(constantText.bulk_dashboard_route)
              }
            >
              <AngleLeftArrow />
            </span>
            <span data-test="ingestion-heading-text">
              {constantText.view_job_text}
            </span>
          </div>
          <div className="master-form s-form flex align-items-center">
            {allRows?.length ? (
              <div className="flex">
                ...
                <BadgeBox status={`${failedCount} Failed`} color={"darkRed"} />
              </div>
            ) : null}
            <input
              type="text"
